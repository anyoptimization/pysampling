...
def centered_l2_discrepancy(X):
    ...
    s2 = (
        (1 + 0.5 * cX[:, None] + 0.5 * cX[None, :] - 0.5 * np.abs(X[:, None] - X[None, :])).prod(
            axis=2
        )
    ).sum()
...
def l2_discrepancy(X):
    ...
    s2 = np.sum(
        np.prod(
            (1 - np.maximum(X[:, None], X[None, :])) * np.minimum(X[:, None], X[None, :]), axis=2
        )
    )
    return np.sqrt(
        12 ** (-n_dim) - (((2 ** (1 - n_dim)) / n_points) * s1) + ((1 / n_points**2) * s2)
    )
...
def l2_star_discrepancy(X):
    ...
    t2 = (1 - X).prod(axis=1) / (n_points**2) - ((2 ** (1 - n_dim)) / n_points) * (1 - X**2).prod(
        axis=1
    )
...
def modified_l2_discrepancy(X):
    ...
    return np.sqrt(
        ((4 / 3) ** n_dim) - (((2 ** (1 - n_dim)) / n_points) * s1) + ((1 / n_points**2) * s2)
    )
