...
- Fix the *cause* of a failing test, not the assertion — use tolerances for
  floats and a pinned `random_state` for stochastic tests
  (`sample(..., random_state=...)`; accepts an int seed or a numpy `Generator`).
...
- Exact per-method outputs are pinned by golden baselines (`tests/test_golden.py`,
  `tests/golden/`). Run `pyclawd golden` after changing any algorithm; bless an
  intended change with `pyclawd golden update` and review the baseline diff.
